...
- `base/kustomization.yaml` — digest-pins the operator image (`:1.30.0@sha256:a2701…efebb`, the multi-arch manifest digest from GHCR) and sets `OPERATOR_IMAGE_NAME` to the same pinned reference.

Security posture is upstream's own: the manager runs as UID 10001 with `ALL` capabilities dropped, read-only root filesystem, and `RuntimeDefault` seccomp — compatible with the `database` namespace's restricted PSA.
