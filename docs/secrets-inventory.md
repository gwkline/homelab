...
| --- | --- | --- | --- | --- | --- |
| C1 | 1Password service-account token | Secret `onepassword-service-account` in `agents`, `sandbox` **and** `tailscale` (bootstrap-only `scripts/create-onepassword-secret.sh` — idempotent, token via env/stdin, never logged; see `docs/rebuild-runbook.md`) | `token` | The token itself lives only in 1Password's non-homelab storage / operator device; it authorizes the dedicated `homelab` vault **only** — least privilege by construction | The only hand-entered secret in the stack. Consumers: the `SecretStore` `onepassword` in each namespace. Rotation owner: operator (create a new SA, re-run the script, ESO re-authenticates on next refresh) |
