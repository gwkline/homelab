...
- **Job shapes in `sandbox` are admission-constrained** (`deploy/sandbox-policy/base`, issue #28): a native ValidatingAdmissionPolicy/CEL ruleset denies hostPath/hostPID/hostIPC/hostNetwork/hostPort, non-allowlisted ServiceAccounts and token mounts, privileged containers outside the exact dind shape, foreign images, PVCs, writable secret mounts, and resource blowouts — so Job-create RBAC is no longer implicit node administration. Break-glass for operators: label a Job `sandbox.gwkline.io/break-glass: "true"` (checked against cluster-admin credentials, so agents cannot use it); details in `deploy/sandbox-policy/README.md`.
...
- **Pods cannot talk to Kubernetes** — with five deliberate, audited exceptions. Dispatcher (label-driven issue automation) and the panel (control-panel backend) hold Job/CronJob write verbs scoped to `sandbox` and nothing else (each ServiceAccount lives in the namespace its pods run in: dispatcher's in `sandbox`, panel's in `agents`); hermes holds **read-only** access (self-visibility + cluster health) and requests factory work through the Executor MCP gateway, which carries the write credentials host-side behind profile validation and approval policy; alloy (cluster log collector), read-only `pods` list/watch plus `pods/log` get cluster-wide so completed jobs' logs survive pod deletion (`deploy/loki/README.md`); and Headlamp (`deploy/headlamp/base`), the Kubernetes web UI acting as its pod's ServiceAccount under a get/list/watch-only cluster role. None can touch secrets, nodes, CRDs, or anything in their own namespace. A mounted CLUSTER.md playbook teaches hermes the governed patterns.
...
- **Supply chain: pinned, signed, verified — three different guarantees.**
...
CI publishes images to `ghcr.io/gwkline/homelab/{t3code,loop-agent,hermes,...}` on every push to `main`. Each build gets an immutable `sha-<commit>` tag and a `latest` tag that CI overwrites; the `latest` tag is only a lookup anchor — manifests and job-spawning code reference the images by `@sha256` digest, so a rebuild or restart can never swap code without a reviewed diff. Renovate watches the `latest` digest and opens the bump PR that moves a workload to a new build (auto-merged once CI passes); `deploy/auto-deploy` then applies the updated manifests to the cluster. If you fork this repo, update the image paths (or add a kustomize overlay with your own `images:` transforms).
